...
def get_main_menu() -> InlineKeyboardMarkup:
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(text="مدیران فرعی", callback_data="panel_admins"),
            InlineKeyboardButton(text="آمار لیست سفید", callback_data="panel_whitelist")
        ],
        [
            InlineKeyboardButton(text="راهنمای دستورات", callback_data="panel_help")
        ],
        [
...
async def view_chats(callback: CallbackQuery, is_tenant_owner: bool = False, tenant_id: int = None, is_owner: bool = False):
    ...
    if not chats:
        ...
        for idx, chat in enumerate(chats, 1):
            chat_type_str = "گروه" if chat.chat_type in ["group", "supergroup"] else "کانال"
            log_status = "دارد" if chat.log_chat_id else "ندارد (/setlog)"
            text += f"{idx}. {chat_type_str} | آیدی چت: {chat.chat_id}\n   وضعیت کانال لاگ: {log_status}\n\n"
...
async def view_admins(callback: CallbackQuery, is_tenant_owner: bool = False, tenant_id: int = None, is_owner: bool = False):
    ...
    if is_owner and (tenant_id is None or tenant_id == 1):
        text = "مدیران فرعی (محیط تست اونر)\n\nهنوز هیچ مجموعه‌ای ثبت نشده است."
        back_button = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="بازگشت", callback_data="panel_main")]])
        try:
            await callback.message.edit_text(text, reply_markup=back_button)
        except TelegramBadRequest:
    ...
    text = "لیست مدیران فرعی ربات (دسترسی سفید کردن):\n\n"
    if not admins:
        text += "هیچ ادمین فرعی تعریف نشده است.\nبرای افزودن، روی کاربر مورد نظر دستور /addadmin را بزنید."
    else:
...
async def view_whitelist_stats(callback: CallbackQuery, is_tenant_owner: bool = False, tenant_id: int = None, is_owner: bool = False):
    ...
    text = (
        ...
        f"برای ثبت کاربر جدید از دستور /register و برای بن همگانی از /unregister روی ریپلای شخص استفاده کنید."
    )
...
async def view_help_menu(callback: CallbackQuery, is_tenant_owner: bool = False, is_owner: bool = False):
    """نمایش راهنمای جامع دستورات برای خریدار"""
    if not is_tenant_owner and not is_owner:
        await callback.answer("دسترسی غیرمجاز", show_alert=True)
        return

    text = (
        "راهنمای جامع دستورات ربات:\n\n"
        "دستورات مدیریت مجموعه (فقط برای خریدار اصلی):\n"
        "- /panel : باز کردن منوی مدیریت\n"
        "- /addchat : اتصال گروه یا کانال فعلی به دیتابیس مجموعه شما\n"
        "- /setlog : تنظیم چت فعلی به عنوان محل ارسال لاگ‌های امنیتی\n"
        "- /addadmin : افزودن کاربر ریپلای شده به عنوان ادمین فرعی ربات\n"
        "- /removeadmin : حذف ادمین فرعی (با ریپلای)\n\n"
        "دستورات کنترل دسترسی (برای خریدار اصلی و ادمین‌های فرعی):\n"
        "- /register : ثبت کاربر ریپلای شده در لیست سفید جهت جلوگیری از حذف شدن\n"
        "- /unregister : حذف کاربر ریپلای شده از لیست سفید\n\n"
        "دسترسی‌ها به صورت کاملاً ایزوله برای مجموعه شما ذخیره و پردازش می‌شوند."
    )
